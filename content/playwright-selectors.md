...
- **Pros:** matches what the user/screen-reader perceives; survives class/tag refactors; failing tests signal a real behavior change; doubles as an accessibility check (the element *must* be in the a11y tree to match).
...
- **Cons:** tightly coupled to implementation details — class renames and DOM restructures cause false failures; tests no longer track the *goal* of the code. XPath especially is verbose and fragile.
