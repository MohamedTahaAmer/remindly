...
Most suites start as "one config, one browser, run everything." That breaks the moment something must happen *before* the tests — seeding a DB, logging in, generating fixtures. Stuffing that into `beforeAll` and hoping the order works out is the fragile path. **Projects** make the ordering structural.
...
`teardown` is its counterpart — a project that runs *after* all dependents finish, for cleanup that would otherwise leak across runs:
...
- `--project=chromium --no-deps` — runs *only* chromium, skipping setup/teardown. A local-iteration shortcut, not for CI.
...
- **Don't create a project per test file.** Projects are *configuration boundaries* (browser, role, environment). Files sharing a browser + `storageState` belong in one project.
