...
import { askClaude } from "@remindly/ai"
import { serverConfig } from "@remindly/config"
import { Logger, run } from "@remindly/utils"
...
export class VideoAgentService {
	...
	private async analyze(state: ProjectState): Promise<{ cuts: Array<AiCut>; flagged: boolean }> {
		...
		// one-shot structured call on the claude CLI's subscription auth; the
		// cutListSchema is enforced on the call itself — see @remindly/ai
		const output = await askClaude({ system: EDIT_POLICY, prompt: numberedTranscript, schema: cutListSchema })

		// validation: bounds, echo check (the hallucination guard), sort, merge overlaps
		const valid = output.cuts.filter((c) => {
